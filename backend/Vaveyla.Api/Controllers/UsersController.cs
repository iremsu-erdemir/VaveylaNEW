        ...
        CancellationToken cancellationToken)
    {
        ...
        await using (var stream = file.OpenReadStream())
        {
            var moderationResult = await _imageModerationService.CheckAsync(
                stream,
                file.ContentType,
                ImageModerationPurpose.ProfilePhoto,
                cancellationToken);
            if (!moderationResult.Allowed)
            {
                var message = moderationResult.Reason switch
                {
                    "nsfw_detected" =>
                        "Uygunsuz içerik tespit edildi. Profil fotoğrafı olarak çıplaklık veya cinsel içerik kullanılamaz.",
                    "moderation_disabled" or "moderation_unavailable" or "invalid_response" =>
                        "Profil fotoğrafı güvenlik kontrolünden geçirilemedi. Lütfen daha sonra tekrar deneyin.",
                    _ => "Fotoğraf yüklenemedi. Lütfen tekrar deneyin.",
                };

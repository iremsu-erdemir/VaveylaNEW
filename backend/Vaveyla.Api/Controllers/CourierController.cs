...
public sealed class CourierController : ControllerBase
{
        ...
        CancellationToken cancellationToken)
    {
        ...
        var declinedOrders = await _ordersRepo.GetOrdersDeclinedByCourierAsync(
            courierUserId,
            cancellationToken);
        var seenOrderIds = orders.Select(o => o.OrderId).ToHashSet();
        foreach (var declined in declinedOrders)
        {
            if (seenOrderIds.Add(declined.OrderId))
            {
                orders.Add(declined);
            }
        }

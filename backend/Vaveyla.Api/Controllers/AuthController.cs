...
public sealed class AuthController : ControllerBase
{
        ...
        CancellationToken cancellationToken)
    {
        ...
        var rowsAffected = await _users.UpdatePasswordAndClearResetAsync(
            user.UserId,
            passwordHash,
            usedTokenHash,
            cancellationToken);
        ...
        {
            _logger.LogWarning(
                "Şifre sıfırlama güncellemesi hiçbir satırı etkilemedi. Email={Email}, UserId={UserId}",
                email,
                user.UserId);
            return StatusCode(500, new { message = "Şifre güncellenemedi. Lütfen tekrar deneyin." });
        }

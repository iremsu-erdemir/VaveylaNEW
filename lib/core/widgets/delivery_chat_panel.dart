...
class _DeliveryChatPanelState extends State<DeliveryChatPanel> {
  ...
  Future<void> _send() async {
    ...
    if (uid.isEmpty) {
      setState(
        () => _sendError = 'Mesaj göndermek için oturum açmalısınız.',
      );
      return;
    }
    final profanityError = ProfanityValidator.validateOrNull(text);
    if (profanityError != null) {
      setState(() => _sendError = profanityError);
      return;
    }
    setState(() {
      _sending = true;
    ...
    try {
      ...
      if (!mounted) return;
      final message = localizeFeedbackMessage(e);

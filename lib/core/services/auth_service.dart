...
class AuthService {
  ...
  Future<http.Response> _postWithFallback({
    ...
    for (final baseUrl in ordered) {
      try {
        ...
        if (retryOnUnauthorized && response.statusCode == 401) {
          ...
          triedUnauthorizedHosts++;
          if (kDebugMode && triedUnauthorizedHosts < ordered.length) {
            debugPrint(
              'AuthService 401 ($baseUrl) — sonraki API adresi deneniyor.',
            );
          }
          continue;
        }
    ...
    if (unauthorizedResponse != null) {
      if (kDebugMode && retryOnUnauthorized) {
        debugPrint(
          'AuthService: Tüm API adreslerinde kimlik doğrulama başarısız '
          '(${ordered.join(', ')}). E-posta veya şifre hatalı.',
        );
      }

...
class _ForgotPasswordScreenState extends State<ForgotPasswordScreen> {
  ...
  String _mapRequestCodeError(Object error, String email) {
    if (error is AuthException) {
      final msg = error.message.trim();
      if (msg.isNotEmpty) return msg;
    }
    final raw = error.toString().toLowerCase();
    if (raw.contains('404') || raw.contains('bulunamad')) {
      return 'Bu e-posta ile kayıtlı hesap bulunamadı. Kayıt olurken kullandığınız '
          'adresi girin: $email';
    }
    ...
    return 'Kod gönderimi başarısız oldu. Lütfen tekrar deneyin.';
  }
  ...
  Future<void> _requestCode() async {
    ...
    try {
      ...
      final message = _mapRequestCodeError(error, email);
  ...
  Widget build(BuildContext context) {
    ...
    return PopScope(
      canPop: _step != _ForgotPasswordStep.done,
      onPopInvokedWithResult: (didPop, result) {
        if (didPop || _step != _ForgotPasswordStep.done) {
          return;
        }
        _returnToLoginAfterReset();
      },
      child: Scaffold(
      body: Container(
      ...
      case _ForgotPasswordStep.done:
        return Column(
          ...
          children: [
            ...
            const SizedBox(height: Dimens.largePadding),
            Text(
              '${_emailController.text.trim()} hesabının şifresi güncellendi.',
              textAlign: TextAlign.center,
            ...
            AppButton(
              title: 'Girişe Dön',
              onPressed: _returnToLoginAfterReset,
              margin: EdgeInsets.zero,
              borderRadius: 14,
              textStyle: typography.titleMedium.copyWith(
                color: colors.white,

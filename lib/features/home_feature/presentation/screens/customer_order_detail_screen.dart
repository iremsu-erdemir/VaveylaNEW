...
class _CustomerOrderDetailScreenState extends State<CustomerOrderDetailScreen> {
  ...
  Widget _actions(OrderDetailModel d) {
    ...
    if (d.showReorderButton) {
      addButton(
        AppButton(
          title: 'Tekrar Sipariş Ver',
          onPressed: d.isReorderEnabled ? () => _reorder(d) : null,
          margin: EdgeInsets.zero,
        ),
      );
    }

    if (d.showCancelButton) {
      final enabled = d.isCancelEnabled;
      addButton(
        AppButton(
          ...
          textStyle: typography.labelLarge.copyWith(
            color: enabled ? colors.white : colors.error.withValues(alpha: 0.6),
          ),
          onPressed: enabled ? () => _cancelOrder(d) : null,
          margin: EdgeInsets.zero,
        ),
      );
    }
    ...
    if (d.showInvoiceButton) {
      final enabled = d.isInvoiceEnabled;
      addButton(
        AppButton(
          ...
          onPressed: enabled ? () => _viewInvoice(d) : null,
          margin: EdgeInsets.zero,
        ),
      );
    }

...
class _SplashScreenState extends State<SplashScreen> {
  ...
  void _clearLoginFields({bool showResetHint = false, String? email}) {
    final resetEmail = email?.trim() ?? '';
    setState(() {
      _loginFormGeneration++;
    ...
    if (showResetHint && mounted && resetEmail.isNotEmpty) {
      _showMessage(
        message:
            '$resetEmail için şifreniz güncellendi. Yeni şifrenizi girin.',
  ...
  Future<void> _openForgotPasswordScreen() async {
    ...
    final resetEmail = await appPush(
      context,
      ForgotPasswordScreen(initialEmail: email),
    );
    if (!mounted || resetEmail is! String || resetEmail.trim().isEmpty) {
      return;
    }
    _clearLoginFields(showResetHint: true, email: resetEmail);
  }
                    ...
                    child: ConstrainedBox(
                      ...
                      child: Column(
                        ...
                        children: [
                          ...
                          _LoginInputField(
                            key: ValueKey('login-email-$_loginFormGeneration'),
                            hintText: 'E-posta',
                            icon: Icons.person,
                          ...
                          SizedBox(height: fieldSpacing),
                          _LoginInputField(
                            key: ValueKey('login-password-$_loginFormGeneration'),
                            hintText: 'Şifre',
                            icon: Icons.lock,
